...
def match_loftr(img1, img2):
    ...
    return result['keypoints0'].cpu().numpy(), result['keypoints1'].cpu().numpy()


def filter_by_mask(pts1, pts2, mask1, mask2):
    valid = []
    for i, (p1, p2) in enumerate(zip(pts1, pts2)):
        ...
        if (0 <= y1 < mask1.shape[0] and 0 <= x1 < mask1.shape[1] and
            ...
            valid.append(i)
    return (pts1[valid], pts2[valid]) if valid else (pts1[:0], pts2[:0])
